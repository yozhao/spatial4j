...
public class GlobalMercator {
  ...
  public GlobalMercator(int tilesize) {
    ...
    this.initialResolution = 2 * Math.PI * 6378137 / (double)tilesize;
  ...
  public int[] MetersToTile(double mx, double my, int zoom) {
    int[] vv = this.MetersToPixels( mx, my, zoom);
    return this.PixelsToTile( vv[0], vv[1] );
  }
  ...
  public int ZoomForPixelSize(double pixelSize ) {
    for( int i=0;i<30; i++) {
      if( pixelSize > this.Resolution(i) ) {
        if(--i<0) // We don't want to scale up
          return 0;
        return i; 
      }
    }
